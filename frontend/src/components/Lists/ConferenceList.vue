...
import { useQueryAll } from '@/_Composables/GraphQL/Conference/useQueryAll';
import ConferenceRow from '@/components/Lists/ConferenceRow.vue';
...
<template>
	<q-list class="p-2 flex flex-col gap-2">
		<q-item-label header class="font-semibold px-1">Active Conferences</q-item-label>
		<div v-if="allError" class="px-3">
			<q-banner inline-actions class="text-white bg-red" rounded>
				<div class="flex flex-row gap-4 items-center">
					<MDIAlert class="w-6 h-6" />
					<div class="flex flex-col gap-0.5">
						<div class="font-bold">There was an error loading rooms.</div>
		...
		<div class="flex flex-col gap-2">
			<ConferenceRow class="mx-3" v-for="(row, rowIdx) in all" :key="rowIdx" :value="row || undefined" />
			...
			<div v-if="!any && !allLoading" class="px-3">
				<q-banner inline-actions class="text-black bg-grey-2" rounded>
					<div class="flex flex-row gap-4 items-center">
						...
						<div class="flex flex-col gap-0.5">
							<div class="font-bold">No Rooms</div>
						</div>
					</div>
				</q-banner>
			</div>
